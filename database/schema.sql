...
-- Configurable AI sales agents. Product knowledge and sales behavior are scoped to
-- an agent so an ERP agent can never load a Hotel CRM knowledge base.
...
CREATE TABLE IF NOT EXISTS agent_knowledge (
    ...
    tags JSON DEFAULT '[]', stage_keys JSON DEFAULT '[]', intent_keys JSON DEFAULT '[]',
    priority INT DEFAULT 0, active BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS agent_intent_rules (
    id SERIAL PRIMARY KEY, agent_id INT REFERENCES sales_agents(id) ON DELETE CASCADE,
    intent VARCHAR(80) NOT NULL, description TEXT, examples JSON DEFAULT '[]',
    priority INT DEFAULT 0, active BOOLEAN NOT NULL DEFAULT TRUE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS agent_stage_rules (
    ...
    stage_key VARCHAR(80) NOT NULL, stage_name VARCHAR(255) NOT NULL, objective TEXT NOT NULL,
    stage_order INT NOT NULL, active BOOLEAN NOT NULL DEFAULT TRUE,
    UNIQUE(agent_id, stage_key), UNIQUE(agent_id, stage_order)
);
CREATE TABLE IF NOT EXISTS conversation_memories (
    id SERIAL PRIMARY KEY, lead_id INT NOT NULL REFERENCES hotel_leads(id) ON DELETE CASCADE,
    agent_id INT NOT NULL REFERENCES sales_agents(id) ON DELETE CASCADE,
    summary TEXT, current_stage VARCHAR(80), lead_score INT DEFAULT 0,
    pain_points JSON DEFAULT '[]', interested_features JSON DEFAULT '[]', decision_maker VARCHAR(255),
    objections JSON DEFAULT '[]', budget VARCHAR(255), timeline VARCHAR(255), next_objective TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
